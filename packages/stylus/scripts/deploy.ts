...
export default async function deployScript(deployOptions: DeployOptions) {
  ...
  // ChambaChain: el deployer queda como owner del registro y, por el constructor,
  // tambien como primer oracle autorizado. El backend firma con esa misma llave
  // (ORACLE_PRIVATE_KEY) para poder escribir atestaciones.
  await deployStylusContract({
    contract: "reputation-registry", // folder name under packages/stylus/contracts/
    constructorArgs: [config.deployerAddress!], // initial_owner
    ...deployOptions,
  });
